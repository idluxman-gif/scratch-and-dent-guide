 ...
 *
 * Sends personalized premium listing emails to shop owners.
 * Supports both ScratchAndDentGuide and ShieldFinder via --site flag.
 ...
 *   node scripts/send-premium-outreach.js --site sad --input recipients.csv --dry-run
 *   node scripts/send-premium-outreach.js --site sad --input recipients.csv --limit 10
 *
 * CSV format (header row optional):
 *   shopName,email
 *   Spencer's TV & Appliance,owner@spencers.com
 *   Good Deals Appliances,info@gooddealsappliances.com
 *
 *   Optional third column for explicit slug override:
 *   shopName,email,slug
 *   Good Deals Appliances,info@gooddealsappliances.com,good-deals-appliances-ft-myers
 *
...
async function main() {
  ...
  if (!args.input || !args.site) {
    console.error('Usage: node scripts/send-premium-outreach.js --site <sad|shieldfinder> --input recipients.csv [--dry-run] [--limit N]');
    console.error('');
    console.error('  --site sad           Send for ScratchAndDentGuide.com');
    console.error('  --site shieldfinder  Send for ShieldFinder.com');
    console.error('');
  ...
  if (!siteConfig) {
    console.error(`Error: Unknown site "${args.site}". Valid options: sad, shieldfinder`);
    process.exit(1);
  }
