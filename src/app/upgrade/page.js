  ...
  if (submitted) {
    return (
      <div style={{ maxWidth: 520, margin: '60px auto', padding: '0 24px', textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: 26, fontWeight: 800, color: '#0F172A', margin: '0 0 12px' }}>Request received!</h2>
        <p style={{ fontSize: 16, color: '#475569', margin: '0 0 24px', lineHeight: 1.6 }}>
          ...
          Once you complete the payment, your listing upgrades automatically within 1 hour.
        </p>
        <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 10, padding: 20, textAlign: 'left' }}>
          <p style={{ margin: 0, fontSize: 14, color: '#166534', fontWeight: 600 }}>What to expect:</p>
          <ol style={{ margin: '8px 0 0', paddingLeft: 20, color: '#166534', fontSize: 14, lineHeight: 2 }}>
        ...
        <Link href="/" style={{ display: 'inline-block', marginTop: 24, color: '#10B981', fontWeight: 600, textDecoration: 'none' }}>
          Back to directory
        </Link>
      </div>
    );
  ...
  return (
    <div style={{ maxWidth: 560, margin: '0 auto', padding: '40px 24px' }}>
      <div style={{ textAlign: 'center', marginBottom: 36 }}>
        ...
        <h1 style={{ fontSize: 30, fontWeight: 800, color: '#0F172A', margin: '0 0 10px' }}>
          Premium Listing
        </h1>
        <p style={{ fontSize: 16, color: '#64748B', margin: 0 }}>
          Stand out from the competition on Scratch & Dent Guide
        </p>
      </div>
      ...
      {shopName && (
        <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 8, padding: 14, marginBottom: 20, fontSize: 15, color: '#166534' }}>
          Upgrading: <strong>{decodeURIComponent(shopName)}</strong>
        </div>
      )}

      {/* Pricing card */}
      <div style={{ background: '#fff', border: '2px solid #10B981', borderRadius: 14, padding: 28, marginBottom: 28, boxShadow: '0 4px 20px rgba(16,185,129,0.1)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#10B981', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>Premium Plan</div>
            <div style={{ fontSize: 34, fontWeight: 800, color: '#0F172A' }}>$29<span style={{ fontSize: 15, fontWeight: 500, color: '#64748B' }}>/month</span></div>
          </div>
      ...
      {!shopId && (
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 14, color: '#64748B', marginBottom: 14 }}>
            To upgrade, visit your store page and click the upgrade button.
          </p>
          <Link href="/" style={{ color: '#10B981', fontWeight: 600, textDecoration: 'none' }}>
            Browse directory →
          </Link>
        </div>
      )}
