...
export default async function StatePage({ params }) {
  ...
  let stateStores = stores.filter(s => s.s === stateCode);
  const premiumChecks = await Promise.all(stateStores.map(s => isPremium(s.i)));
  const premiumSet = new Set(stateStores.filter((s, idx) => premiumChecks[idx]).map(s => s.i));
  stateStores = [...stateStores].sort((a, b) => {
  ...
  const jsonLd = {
    ...
    "itemListElement": stateStores.map((store, idx) => ({
      "@type": "ListItem",
      "position": idx + 1,
